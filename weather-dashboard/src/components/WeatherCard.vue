  ...
  <a-card class="max-w-xl mx-auto">
    ...
    <template v-else>
      <div v-if="error" class="space-y-2">
        <a-alert type="error" show-icon message="خطا در واکشی اطلاعات" :description="String(error)" />
      </div>
      ...
      <div v-else-if="temp !== null" class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-3 pt-4 text-sm">
          <div class="p-3 rounded-xl bg-gray-50">
            حداکثر: <strong>--°</strong> <!-- WeatherAPI.current.json دما مین/ماکس ندارد -->
          </div>
          <div class="p-3 rounded-xl bg-gray-50">
            حداقل: <strong>--°</strong> <!-- WeatherAPI.current.json دما مین/ماکس ندارد -->
          </div>
          <div class="p-3 rounded-xl bg-gray-50">
            رطوبت: <strong>{{ humidity }}%</strong>
          </div>
          <div class="p-3 rounded-xl bg-gray-50">
            باد: <strong>{{ windSpeed }} km/h</strong> <!-- km/h در WeatherAPI -->
          </div>
        </div>
      </div>
